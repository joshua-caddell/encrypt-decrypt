...
int main(int argc, char *argv[])
{
	...
	char *host = "localhost";
    	char buffer[256];
	char key[71000], message[70000], decrypted[70000], keyfile[32], messagefile[32];
	...
	if (strlen(key) < strlen(message))
		error("ERROR: decryption key too short");
	...
	if(strcmp(buffer, "decrypt") != 0)
		error("ERROR: not allowed connect to otp_enc_d ");
	...
	//recevie the decrypted text back from server
	memset(message, 0, sizeof(message));
	char buf[1025];
